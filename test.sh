...
if [[ "$DOMAIN_MODE" == "image_space" ]]; then
    YAML_FILE="test_image.yml"
elif [[ "$DOMAIN_MODE" == "latent_space" ]]; then
    if [[ "$PRIOR_DIM" != "256" ]]; then
        case "$PRIOR_DIM" in
            64) YAML_FILE="latent_prior64_100k.yml" ;;
            128) YAML_FILE="latent_prior128_100k.yml" ;;
            512) YAML_FILE="latent_prior512_100k.yml" ;;
            *) echo "Error: No matching YAML file for prior_dim='${PRIOR_DIM}'"; exit 1 ;;
        esac
    elif [[ "$PRIOR_DIM" == "256" ]]; then
        case "$TIMESTEPS" in
            1) YAML_FILE="timesteps_steps1_100k.yml" ;;
            2) YAML_FILE="timesteps_steps2_100k.yml" ;;
            4) YAML_FILE="timesteps_steps4_100k.yml" ;;
            6) YAML_FILE="timesteps_steps6_100k.yml" ;;
            8) YAML_FILE="timesteps_steps8_100k.yml" ;;
            *) echo "Error: No matching YAML file for timesteps='${TIMESTEPS}'"; exit 1 ;;
        esac
    else
        # Default latent case (prior_dim=256, timesteps=4)
        YAML_FILE="latent_prior256_100k.yml"
    fi
else
...
OPT_PATH="options/pd_fs_ablation/test/${YAML_FILE}"
...
CUDA_VISIBLE_DEVICES=0 python DiffMSR_Main/test.py -opt "$OPT_PATH"
