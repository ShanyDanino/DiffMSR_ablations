...
if [[ "$DOMAIN_MODE" == "image_space" ]]; then
    YAML_FILE="train_S2_image.yml"
elif [[ "$DOMAIN_MODE" == "latent_space" ]]; then
    if [[ "$PRIOR_DIM" != "256" ]]; then
        case "$PRIOR_DIM" in
            64) YAML_FILE="latent_prior64_stage2_100k.yml" ;;
            128) YAML_FILE="latent_prior128_stage2_100k.yml" ;;
            512) YAML_FILE="latent_prior512_stage2_100k.yml" ;;
        ...
        case "$TIMESTEPS" in
            ...
            4) YAML_FILE="timesteps_steps4_stage2_100k.yml" ;;
        ...
        YAML_FILE="latent_prior256_stage2_100k.yml"
    fi
else
