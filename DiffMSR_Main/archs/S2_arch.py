...
sys.path.append('/disk1/lgy/CVPR2024/DiffMSR_Main-SRGAN/DiffMSR_Main/archs/')
import DiffMSR_Main.archs.common as common
import DiffMSR_Main.archs.CATL as CATL
from ldm.ddpm import DDPM
# import DiffMSR_Main.archs.attention as attention
import torch
...
class PG_FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, prior_dim=256):
        ...
        self.kernel = nn.Sequential(
            nn.Linear(prior_dim, dim*2, bias=False),
        )
...
class PL_MSA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        # self.permuted_window_size = (window_size[0] // 8, window_size[1] // 8)
        ...
        # self.kv = nn.Linear(dim, dim // 8, bias=qkv_bias)
    ...
    def forward(self, x, k_v, b, windows_size, mask=None):
        ...
        # kv = self.kv(x).reshape(b_, self.permuted_window_size[0], 8, self.permuted_window_size[1], 8, 8, c // 64).permute(0, 1, 3, 5, 2, 4, 6).reshape(b_, n // 64, 8, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim,
        ...
        self.ffn = PG_FeedForward(dim, ffn_expansion_factor, bias, prior_dim=prior_dim)
...
class BasicLayer(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, num_blocks, prior_dim=256):
        ...
        self.blocks = nn.ModuleList(
            [TransformerBlock(dim=dim, num_heads=num_heads, ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                              LayerNorm_type=LayerNorm_type, prior_dim=prior_dim) for i in
             range(num_blocks)])
...
class PE(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6,scale=4, prior_dim=256):
...
class denoise(nn.Module):
    def __init__(self,n_feats = 64, n_denoise_res = 5,timesteps=5, prior_dim=None):
        super(denoise, self).__init__()
        self.max_period=timesteps*10
        self.prior_dim = int(prior_dim) if prior_dim is not None else 4 * n_feats
        resmlp = [
            nn.Linear(self.prior_dim * 2 + 1, self.prior_dim),
            nn.LeakyReLU(0.1, True),
        ]
...
class DiffMSR_S2(nn.Module):
    def __init__(self,         
        ...
        self.condition = PE(n_feats=64, n_encoder_res=n_encoder_res,scale = scale, prior_dim=prior_dim)

        self.denoise= denoise(n_feats=64, n_denoise_res=n_denoise_res,timesteps=timesteps, prior_dim=prior_dim)
...
#     import time
#     model = DiffMSR_S2(n_encoder_res=9, inp_channels=2, out_channels=2, dim=32, num_blocks=[6,6,6,6], num_refinement_blocks=6,
...
#     IPRS1 = torch.randn(1,256)
#
#     # time_begin = time.time()
#     sr = model(img, ref, IPRS1)
#     # time_end = time.time()
